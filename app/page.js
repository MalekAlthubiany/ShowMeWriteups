...
export default function Page() {
  ...
  const t = useMemo(() => isArabic ? ({
    title:'BugDaily', subtitle:'تقارير علنية يومية لثغرات حرجة في برامج المكافآت', search:'ابحث...', severity:'الشدة',
    all:'الكل', platform:'المنصة', refresh:'تحديث', view:'عرض', live:'تحديث تلقائي', loadMore:'تحميل المزيد',
    no:'لا توجد نتائج'
  }) : ({
    title:'BugDaily', subtitle:'Daily public write-ups of critical bounty reports', search:'Search…', severity:'Severity',
    all:'All', platform:'Platform', refresh:'Refresh', view:'View', live:'Live auto-update', loadMore:'Load more',
    no:'No results'
  }), [isArabic]);
  ...
  useEffect(() => {
    ...
    typing.current = setTimeout(() => { setPage(1); fetchPage(1, true); }, 400);
  ...
  useEffect(() => {
    const id = setInterval(() => { setPage(1); fetchPage(1, true); }, 60000);
        ...
        <div style={{maxWidth:960, margin:'0 auto', padding:'0 16px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12}}>
          <div style={{display:'flex', alignItems:'center', gap:12}}>
            ...
            <div>
              ...
              <div style={{fontSize:12, color:'#64748b'}}>{t.subtitle} <span style={{marginInlineStart:8, color:'#059669'}}>● {t.live}</span></div>
            </div>
          </div>
          <div style={{display:'flex', gap:8, alignItems:'center'}}>
            ...
            <select value={since} onChange={(e)=>setSince(e.target.value)} style={{padding:'8px', border:'1px solid #cbd5e1', borderRadius:8}}>
              {ranges.map(r => <option key={r.v} value={r.v}>{isArabic ? r.ar : r.en}</option>)}
            </select>
            <button onClick={()=>{ setPage(1); fetchPage(1, true); }} style={{padding:'8px 10px', border:'1px solid #cbd5e1', borderRadius:8, background:'#f8fafc'}}>{t.refresh}</button>
          </div>
        </div>
      </header>
      ...
      <main style={{maxWidth:960, margin:'0 auto', padding:'16px'}}>
                ...
                <div style={{display:'flex', justifyContent:'space-between', gap:12}}>
                  <div style={{textAlign:isArabic?'right':'left'}}>
                    <div style={{display:'flex', flexWrap:'wrap', gap:8, alignItems:'center'}}>
                      ...
                      <span style={{padding:'2px 8px', borderRadius:999, background:'#0f172a', color:'#fff', fontSize:12}}>{r.platform}</span>
                      <span style={{padding:'2px 8px', borderRadius:999, background:r.severity==='critical'?'#dc2626':'#0f172a', color:'#fff', fontSize:12}}>
                    ...
                    <div style={{marginTop:6, fontSize:14, color:'#334155', display:'flex', flexWrap:'wrap', gap:8}}>
                      ...
                      {typeof r.bounty==='number' && <span>• Bounty: {r.currency||'USD'} {r.bounty.toLocaleString(locale)}</span>}
                    </div>
                  </div>
                  <a href={r.url} target="_blank" rel="noreferrer" style={{color:'#4338ca', textDecoration:'underline'}}>{t.view}</a>
                </div>
              </div>
            ))}
          </div>
        ...
        {canLoadMore && (
          <div style={{display:'flex', justifyContent:'center', marginTop:16}}>
            <button disabled={loading} onClick={()=>{ const n=page+1; setPage(n); fetchPage(n); }} style={{padding:'10px 14px', border:'1px solid #cbd5e1', borderRadius:10, background:'#fff'}}>
              {t.loadMore} {loading ? '…' : ''}
            </button>
